// app_movil/app/(tabs)/ventas/index.tsx  (REEMPLAZA el existente)
...
interface DetallePrenda {
  id: string; precioVendido: number;
  prenda: { id: string; codigo: string; marca?: string; tipo: string; foto?: string; precio: number; rebaja?: number; };
}
interface MetodoPagoItem { id: string; metodo: 'EFECTIVO' | 'QR' | 'TARJETA'; monto: number; }
interface Venta {
  id: string; fecha: string; totalCobrado: number;
...
export default function VentasScreen() {
  ...
  const fetchVentas = useCallback(async () => {
    try { const res = await api.get('/ventas'); setVentas(res.data); }
  ...
  const formatDate = (iso: string) => new Date(iso).toLocaleDateString('es-BO', { day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' });
  ...
  const downloadInvoice = async (venta: Venta) => {
    try {
      ...
      txt += '================================\n\n';
      txt += `Codigo: #${code}\n`;
      txt += `Fecha: ${formatDate(venta.fecha)}\n`;
      txt += `Vendedor: ${venta.vendedor.nombreCompleto}\n`;
      txt += `Sucursal: ${venta.sucursal.nombre}\n\n`;
      txt += '--------------------------------\n';
      txt += 'PRENDAS\n';
      txt += '--------------------------------\n';
      venta.detallesPrendas.forEach((dp, i) => {
        txt += `${i + 1}. ${dp.prenda.marca ? dp.prenda.marca + ' - ' : ''}${dp.prenda.tipo}\n`;
        txt += `   Codigo: ${dp.prenda.codigo}\n`;
        txt += `   Precio: Bs ${dp.precioVendido}\n\n`;
      });
      txt += '--------------------------------\n';
      txt += 'PAGO\n';
      txt += '--------------------------------\n';
    ...
    <ScrollView style={[st.container, { backgroundColor: colors.pg }]} contentContainerStyle={st.content} showsVerticalScrollIndicator={false}
      ...
      {filtered.length === 0 ? (
        <View style={st.emptyState}>
          ...
          <Text style={{ color: colors.tx4, fontSize: 13 }}>{searchQuery ? 'Sin resultados' : 'Sin ventas registradas'}</Text>
        </View>
      ) : (
        <View style={{ gap: 12 }}>
          {filtered.map((v) => {
              ...
              <View key={v.id} style={[st.ventaCard, { backgroundColor: colors.cdSolid, borderColor: colors.bd2Solid, ...colors.cardShadow }]}>
                <TouchableOpacity activeOpacity={0.85} onPress={() => setExpandedId(isExp ? null : v.id)} style={st.ventaHeader}>
                  ...
                  <View style={{ position: 'relative', width: count > 1 ? Math.min(count, 3) * 14 + 36 : 48, height: 48 }}>
                    {count === 1 ? (
                      <View style={[st.thumb, { backgroundColor: colors.fiSolid }]}>
                        {v.detallesPrendas[0].prenda.foto ? <Image source={{ uri: v.detallesPrendas[0].prenda.foto }} style={st.thumbImg} /> : <Ionicons name="pricetag-outline" size={20} color={colors.tx4} />}
                      </View>
                    ) : (
                      v.detallesPrendas.slice(0, 3).map((dp, i) => (
                        <View key={dp.id} style={[st.thumbStack, { backgroundColor: colors.fiSolid, borderColor: colors.cdSolid, left: i * 14, zIndex: 3 - i }]}>
                          {dp.prenda.foto ? <Image source={{ uri: dp.prenda.foto }} style={st.thumbImg} /> : <Ionicons name="pricetag-outline" size={14} color={colors.tx4} />}
                        </View>
                      ))
                    )}
                  ...
                  <View style={{ flex: 1, marginLeft: 12 }}>
                    <View style={{ flexDirection: 'row', justifyContent: 'space-between' }}>
                      <Text style={{ color: colors.tx, fontSize: 14, fontWeight: '500' }}>#{code}</Text>
                      <Text style={{ color: colors.acEmerald, fontSize: 15, fontWeight: '600' }}>Bs {v.totalCobrado}</Text>
                    </View>
                    <Text style={{ color: colors.tx4, fontSize: 10, marginTop: 2 }}>{formatDate(v.fecha)}</Text>
                    <View style={{ flexDirection: 'row', gap: 6, marginTop: 4, flexWrap: 'wrap' }}>
                      ...
                      {v.metodosDePago.map((mp) => (
                        <View key={mp.id} style={[st.pill, { backgroundColor: `${getMetodoColor(mp.metodo)}15` }]}>
                          <Ionicons name={getMetodoIcon(mp.metodo)} size={10} color={getMetodoColor(mp.metodo)} />
                          <Text style={{ color: getMetodoColor(mp.metodo), fontSize: 9, marginLeft: 3 }}>{mp.metodo === 'EFECTIVO' ? 'Efec' : mp.metodo}{v.metodosDePago.length > 1 ? ` ${mp.monto}` : ''}</Text>
                        </View>
                      ))}
                    </View>
                  </View>
                  ...
                  <View style={[st.detail, { borderTopColor: colors.bd }]}>
                    {v.detallesPrendas.map((dp) => (
                      <View key={dp.id} style={[st.detailItem, { backgroundColor: colors.fiSolid, borderColor: colors.bd }]}>
                        <View style={[st.detailImg, { backgroundColor: colors.pg }]}>
                          {dp.prenda.foto ? <Image source={{ uri: dp.prenda.foto }} style={st.thumbImg} /> : <Ionicons name="pricetag-outline" size={16} color={colors.tx4} />}
                        </View>
                        <View style={{ flex: 1 }}>
                          <Text style={{ color: colors.tx, fontSize: 12 }}>{dp.prenda.marca ? `${dp.prenda.marca} - ` : ''}{dp.prenda.tipo}</Text>
                          <Text style={{ color: colors.tx4, fontSize: 10 }}>{dp.prenda.codigo}</Text>
                        </View>
                    ...
                    <View style={[st.detailFooter, { borderTopColor: colors.bd }]}>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        <Ionicons name="person-outline" size={12} color={colors.tx4} />
                        <Text style={{ color: colors.tx4, fontSize: 10 }}>{v.vendedor.nombreCompleto}</Text>
                      </View>
                      <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                        <Ionicons name="storefront-outline" size={12} color={colors.tx4} />
                        <Text style={{ color: colors.tx4, fontSize: 10 }}>{v.sucursal.nombre}</Text>
                      </View>
                    </View>
                    {/* Botón descargar factura */}
                    <TouchableOpacity onPress={() => downloadInvoice(v)} activeOpacity={0.7}
                      style={[st.invoiceBtn, { backgroundColor: 'rgba(56,189,248,0.1)', borderColor: 'rgba(56,189,248,0.2)' }]}>
                      <Ionicons name="download-outline" size={16} color={colors.acSky} />
                      <Text style={{ color: colors.acSky, fontSize: 12 }}>Descargar factura</Text>
                    </TouchableOpacity>
                  </View>
                )}
              </View>
