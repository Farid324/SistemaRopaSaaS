...
import { Card } from '../../../src/components/ui/display/card';
...
export default function HomeScreen() {
  ...
  // ── Demo data (será reemplazado por datos reales del backend) ──
  const disponibles = 5;
  const vendidas = 0;
  const totalPrendas = 5;
  const totalIngresos = 0;
  const totalPrendasVendidas = 0;
  ...
  const baseData = useMemo(() => {
    if (periodoStats === 'dia') {
      return Array.from({ length: 7 }, (_, i) => {
        ...
        return { label: d.toLocaleDateString('es', { weekday: 'short' }), ventas: v, ingresos: v * (Math.floor(Math.random() * 150) + 80) };
      });
    } else if (periodoStats === 'semana') {
      return Array.from({ length: 4 }, (_, i) => {
        const v = Math.floor(Math.random() * 20) + 3;
        return { label: `Sem ${i + 1}`, ventas: v, ingresos: v * (Math.floor(Math.random() * 130) + 90) };
      });
    } else if (periodoStats === 'mes') {
      return ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun'].map((m) => {
        const v = Math.floor(Math.random() * 60) + 10;
        return { label: m, ventas: v, ingresos: v * (Math.floor(Math.random() * 120) + 100) };
      });
    } else {
      return ['2022', '2023', '2024', '2025'].map((y) => {
        const v = Math.floor(Math.random() * 100) + 50;
        return { label: y, ventas: v, ingresos: v * (Math.floor(Math.random() * 100) + 150) };
      });
    }
  }, [periodoStats]);
  ...
  const pieData = [
    ...
    { name: 'Reservadas', value: 0, color: '#34d399' },
  ].filter((d) => d.value > 0);
    ...
    <ScrollView style={[styles.container, { backgroundColor: colors.pg }]} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      <LinearGradient colors={['#fb7185', '#f59e0b']} start={{ x: 0, y: 0 }} end={{ x: 1, y: 0 }} style={styles.revenueCard}>
        ...
        <View style={styles.revenueRow}>
          <View style={styles.revenueStat}>
            ...
            <Text style={styles.revenueStatText}>{totalPrendasVendidas} vendidas</Text>
          </View>
          <View style={styles.revenueStat}>
            <View style={styles.dot} />
      ...
      {isOwner && (
        <View style={[styles.planCard, { borderColor: 'rgba(251,113,133,0.15)' }]}>
          ...
          <View style={{ gap: 10 }}>
            <PlanBar label="Sucursales" current={2} max={1} icon="storefront-outline" colors={colors} />
            <PlanBar label="Empleados" current={1} max={2} icon="people-outline" colors={colors} />
            <PlanBar label="Prendas" current={5} max={500} icon="pricetag-outline" colors={colors} />
          </View>
