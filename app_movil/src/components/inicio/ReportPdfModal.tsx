//app_movil/src/components/inicio/ReportPdfModal.tsx
...
export function ReportPdfModal({ visible, colors, generatingPdf, onConfirm, onCancel }: Props) {
    ...
    <Modal visible transparent animationType="fade">
      <View style={rm.overlay}>
        <View style={[rm.card, { backgroundColor: colors.cdSolid, borderColor: colors.bd2Solid }]}>
          <View style={[rm.iconCircle, { backgroundColor: 'rgba(251,113,133,0.15)' }]}>
            <Ionicons name="document-text-outline" size={28} color={colors.acRose} />
          </View>
          <Text style={[rm.title, { color: colors.tx }]}>Reporte PDF</Text>
          <Text style={{ color: colors.tx3, fontSize: 12, marginBottom: 16 }}>Selecciona el periodo del documento</Text>
          ...
          <View style={rm.btnRow}>
            ...
            <TouchableOpacity onPress={handleConfirm} disabled={generatingPdf} activeOpacity={0.85} style={{ flex: 1 }}>
              <LinearGradient colors={['#fb7185', '#e11d48']} style={rm.downloadBtn}>
                {generatingPdf ? (
                  <ActivityIndicator color="#fff" size="small" />
                ) : (
                  <>
                    <Ionicons name="download-outline" size={16} color="#fff" />
                    <Text style={{ color: '#fff', fontSize: 14, fontWeight: '600' }}>Generar PDF</Text>
                  </>
                )}
              </LinearGradient>
            </TouchableOpacity>
